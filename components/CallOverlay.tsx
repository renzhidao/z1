...
export const CallOverlay: React.FC<CallOverlayProps> = ({ user, onHangup, type }) => {
  ...
  // PiP State
  const [isSwapped, setIsSwapped] = useState(false); // false: Main=Local/Remote?, actually Main=Remote, PiP=Local usually
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 z-[100] bg-gray-900 overflow-hidden flex flex-col select-none animate-in fade-in duration-300">
      ...
      <div className="absolute top-0 left-0 right-0 pt-safe-top px-4 pb-4 flex justify-between items-center z-10 text-white pointer-events-none bg-gradient-to-b from-black/60 to-transparent">
        <div 
          onClick={onHangup} // Minimize just closes overlay for now? Or actually minimizes? Let's just hook to onHangup for now or do nothing
      ...
      <div 
        className="absolute w-32 h-48 bg-black rounded-xl overflow-hidden border border-white/20 shadow-2xl z-20 cursor-move touch-none will-change-[left,top]"
        style={{ 
