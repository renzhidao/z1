...
export const initBackend = async () => {
  ...
  console.log('🚀 Initializing M3 Backend...');
  // 动态导入 loader，它会按顺序加载 m3 的所有模块
  // 我们使用 vite 的 import 别名或相对路径
  try {
      // 必须确保 m3-1 目录在 vite 的服务范围内
      await import('../../m3-1/loader.js');
  } catch (e) {
      console.error("Failed to load m3 backend", e);
  }
};
