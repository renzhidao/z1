  ...
  const handleSend = async () => {
    ...
    if (chat.isAi) {
      const history = messages.map(m => ({
        role: m.senderId === currentUserId ? 'user' : 'model' as 'user' | 'model',
        parts: [{ text: m.text }]
      }));
    ...
    <div className="fixed inset-0 bg-[#EDEDED] z-50 flex flex-col animate-in slide-in-from-right duration-300">
      ...
      >
        {messages.map((msg, idx) => {
          ...
          // Green: Normal #95EC69, Dark #89D960
          // White: Normal #FFFFFF, Dark #F2F2F2
          ...
          return (
            <div key={msg.id} className="mb-4 relative">
              {showTime && (
                // 修复：将间距调整为符合垂直韵律的值
                // mb-[18px] (空隙) + 时间本身高度 (~22px) = 40px (气泡标准高度)
                ...
                >
                   {msg.type === 'voice' || msg.text.startsWith('[语音]') ? (
                      ...
                      />
                       ...
                       <img src="https://picsum.photos/seed/chatimg/400/300" className="rounded-[6px] border border-gray-200 max-w-[200px]" alt="Chat Image" />
                   ) : (
                      // 文本气泡
                      <div 
                      ...
                      >
                         {/* 
                            修复：气泡尖角 (Triangle Arrow)
                            使用 style 直接渲染颜色，解决 Tailwind Purge 导致看不见的问题
                         */}
                         <div 
                            className={`absolute top-1/2 -translate-y-1/2 w-0 h-0 border-[6px] border-transparent ${
                                isMe ? 'right-[-6px]' : 'left-[-6px]'
