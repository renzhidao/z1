...
export function init() {
  ...
  style.textContent = `
    ...
    .video-error, .img-error-box {
        color: #ff3b30; font-size: 11px; padding: 10px; text-align: center;
        border: 1px dashed #ff3b30; border-radius: 4px;
    }
    .chat-img.error {
  ...
  const render = {
    ...
    updateSelf() {
      ...
      if (elId && window.state && window.state.myId) elId.innerText = window.state.myId.slice(0, 6);
      if (elNick && window.state) elNick.innerText = window.state.myName || '';

      if (elSt && window.state) {
        let s = '在线';
      ...
      if (elDot && window.state) elDot.className = window.state.mqttStatus === '在线' ? 'dot online' : 'dot';

      if (elCount && window.state && window.state.conns) {
        let count = 0;
        Object.values(window.state.conns).forEach(c => { if (c && c.open) count++; });
        elCount.innerText = String(count);
      }
    },
    ...
    renderList() {
      ...
      map.forEach((v, id) => {
        if (!id || id === window.state.myId || (window.config && window.config.hub && id.startsWith(window.config.hub.prefix))) return;
    ...
    appendMsg(m) {
      ...
      const isMe = (window.state && m.senderId === window.state.myId);
      let content = '';
      let style = '';

      if (m.kind === 'SMART_FILE_UI') {
        const meta = m.meta || {};
        const sizeStr = meta.fileSize ? (meta.fileSize / (1024 * 1024)).toFixed(2) + ' MB' : '';
        ...
        const safeName = window.util.escape(meta.fileName || 'file');
