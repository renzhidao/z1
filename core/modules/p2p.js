  ...
  window.p2p = {
    ...
    setupConn(conn) {
      ...
      conn.on('open', () => {
        ...
        try { if (window.smartCore && typeof window.smartCore.onPeerConnect === 'function') window.smartCore.onPeerConnect(pid); } catch (e) {}
    ...
    handleData(d, conn) {
      ...
      const sc = window.smartCore;

      const callBinary = (payload) => {
        if (!sc) return false;
        const fn = (typeof sc.onBinary === 'function') ? sc.onBinary
          : ((typeof sc.handleBinary === 'function') ? sc.handleBinary : null);
        if (!fn) return false;
        try { fn.call(sc, payload, conn.peer); return true; } catch (e) { return false; }
      };

      if (typeof Blob !== 'undefined' && d instanceof Blob) {
        const reader = new FileReader();
        reader.onload = () => { callBinary(reader.result); };
      ...
      if (String(d.t).startsWith('SMART_')) {
        // API mode: let smartCore consume first (SMART_GET has no id)
        if (sc && sc.mode === 'api' && typeof sc.onPacket === 'function') {
          try { if (sc.onPacket(d, conn.peer)) return; } catch (e) {}
        }
        if (window.protocol) window.protocol.processIncoming(d, conn.peer);
        return;
      }
