...
export function init() {
  ...
  window.smartCore = {
      ...
      play: (fileId, name) => {
          ...
          // === URL 构造修正 ===
          // 确保生成的 URL 以 m3BaseUrl 开头，以便 SW (scoped to m3-1) 可以拦截
          const baseUrl = window.m3BaseUrl || './';
          // Ensure baseUrl has a trailing slash for URL constructor context
          const safeBase = baseUrl.endsWith('/') ? baseUrl : baseUrl + '/';
          
          // 使用 URL 类构造绝对路径，确保 SW 能捕获
          const vUrl = new URL(`virtual/file/${fileId}/${encodeURIComponent(fileName)}`, safeBase).href;
      ...
      download: (fileId, name) => {
          ...
          const baseUrl = window.m3BaseUrl || './';
          const safeBase = baseUrl.endsWith('/') ? baseUrl : baseUrl + '/';
          const url = new URL(`virtual/file/${fileId}/${encodeURIComponent(fileName)}`, safeBase).href;
