...
class SmartCore {
  ...
  play(fileId, name = '') {
    ...
    // 2. 远程文件：确保任务已启动，然后返回 SW 虚拟 URL
    // 如果没有连接 Peer，尝试主动连接
    if (meta.senderId && window.p2p && (!window.state.conns[meta.senderId] || !window.state.conns[meta.senderId].open)) {
  ...
  onPeerConnect(pid) {
    // 节点上线时，通知 TaskManager 检查是否有卡住的任务需要该节点
    if (this.tasks && this.tasks.onPeerConnect) {
        // this.tasks.onPeerConnect(pid); // 暂未实现 task.js 中的具体逻辑，预留
    }
  }
