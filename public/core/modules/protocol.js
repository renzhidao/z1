...
export function init() {
  ...
  window.protocol = {
    ...
    async processIncoming(pkt, fromPeerId) {
      // API 模式下：SMART_* 必须在 "id 检查" 前被 smartCore 处理（因为 SMART_GET 没有 id）
      if (pkt && pkt.t && String(pkt.t).startsWith('SMART_')) {
        if (window.smartCore && window.smartCore.mode === 'api' && typeof window.smartCore.onPacket === 'function') {
          if (window.smartCore.onPacket(pkt, fromPeerId)) return;
        }
      }

      // 注意：这里会过滤掉没有 ID 的包（如原始 SMART_GET），所以上面的 Hook 必须生效
